...
// Export helpers
module.exports.register = function (Handlebars, opt)  {
  ...
  var helpers = {
    ...
    partialFileContext: function(name, contextFile) {
      var ymlFileContent = yaml.safeLoad(fs.readFileSync(contextFile, 'utf8'));
      ymlFileContent.contextFile = contextFile;
      return helpers.partial(name, ymlFileContent);
    },

    partialGlobContext: function(name, pattern) {
      return glob.sync(pattern).reduce(function(html, fileName) {
        html += helpers.partialFileContext(name, fileName);
        return html;
      }, '');
    },
